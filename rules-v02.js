// Commander Lab v0.2.0 — core rules test layer
(() => {
  ...
  function canPay(cost){
    ...
    for(const k of MANA){if(pool[k]<cost[k])return false;pool[k]-=cost[k]}
    return MANA.reduce((s,k)=>s+pool[k],0)>=cost.generic;
  }
  function pay(cost){
  ...
  function castFromHand(c){
    ...
    if(!canPay(cost)){toast(`Not enough mana. Need ${c.mana_cost||'{0}'}.`);return}
    pay(cost);const i=state.hand.findIndex(x=>x._instance===c._instance);if(i<0)return;state.hand.splice(i,1);
  ...
  function rulesOpenCard(c,zone){
    ...
    if(zone==='hand'){
      if(isLand(c)){
        ...
        const cost=parseCost(c),timing=canCastTiming(c),money=canPay(cost),unsupported=cost.unsupported.length>0;addAction(a,`Cast ${c.mana_cost||'{0}'}`,'green',()=>castFromHand(c),!timing||!money||unsupported);
        if(unsupported)addReason(a,`Unsupported mana symbol(s): ${cost.unsupported.map(x=>`{${x}}`).join(' ')}.`);else if(!timing)addReason(a,'This spell is not legal in the current phase.');else if(!money)addReason(a,`Current mana: ${manaText()}.`);else addReason(a,'Card rules text/effects are still manual in v0.2.');
      }
      ...
      const tax=rules.commanderCasts*2,cost=parseCost(state.commander,tax),ok=rules.commanderLocation==='command'&&inMain()&&canPay(cost)&&!cost.unsupported.length;addAction(a,`Cast ${costLabel(state.commander,tax)}`,'green',castCommander,!ok);
      if(rules.commanderLocation!=='command')addReason(a,'Commander is currently on the battlefield.');else if(!inMain())addReason(a,'Cast this creature commander during a main phase.');else if(!canPay(cost))addReason(a,`Current mana: ${manaText()}.`);
  ...
  function phaseHint(){switch(phase()){case'Untap':return'Permanents untap automatically.';case'Upkeep':return'Upkeep triggers are not automated yet.';case'Draw':return'A card is drawn automatically.';case'Main 1':case'Main 2':return'Play a land and cast sorcery-speed spells.';case'Combat':return'Tap an eligible creature, choose Attack, then Resolve Attack.';case'End':return'Mana empties when you leave the phase.';default:return''}}
